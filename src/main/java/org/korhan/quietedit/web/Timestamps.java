 ...
 *
 * <p>It exists because a template otherwise prints an {@code Instant} through its
 * {@code toString()} -- {@code 2026-08-27T13:26:31.574126Z} -- next to a formatted fetch
 * time, two renderings of one kind of value on one page. Keeping the format and the zone
 * in a bean means the next page cannot disagree with these two.
 *
 * <p>{@code format} stops at minutes, which is the resolution an editorial event has,
 * while {@link #exact(Instant)} supplies seconds for a {@code title} attribute -- two
 * revisions of one article can be seconds apart, and minute precision makes them look
 * simultaneous.
 *
 * <p>Null in, null out: an absent timestamp is the template's business.
 */
